...
| `suggest_entity_links` | — | candidate customer/vendor pairs that may be the same business, each with its `reason`: normalised names equal, equal apart from a trailing legal suffix (LLC, Inc, Co, …), or one's `qbo_name` is the other's name. Linked and archived records are left out. Read-only — it links nothing; the operator confirms each with `update_company`. |
