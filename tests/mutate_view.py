...
M = [
 ...
 ("an untouched vendor is re-sent on every save",
  "  if(sv && sv.getAttribute('data-orig') !== null && sv.value !== sv.getAttribute('data-orig'))",
  "  if(sv)"),
 ("the vendor page lists settled legs as open POs",
  "    const openLegs=(DATA.shipments||[]).filter(s=>st(s.vendor_id)===st(selected) && !legSettled(s));",
  "    const openLegs=(DATA.shipments||[]).filter(s=>st(s.vendor_id)===st(selected));"),
 ("an empty Open POs section is rendered",
