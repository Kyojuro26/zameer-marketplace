...
MERGE = [
 ...
 ("the stored value overrides one the workbook supplies",
  "                if merged_rec.get(field) is None and field in prior:\n",
  "                if field in prior:\n"),
 # the operator's decision (0.1.43): a project's customer change carries and
 # is reported; a leg's vendor-PO change drops and is reported
 # re-anchored after the focused review: both compare with the merged record
 ("a customer change is not reported",
  "            if fname == \"projects.json\" and \\\n"
  "                    _s(prior.get(\"company_id\")) != _s(merged_rec.get(\"company_id\")):\n",
  "            if False:\n"),
 ("a company_id differing only by whitespace reads as a customer change",
  "                    _s(prior.get(\"company_id\")) != _s(merged_rec.get(\"company_id\")):\n",
  "                    prior.get(\"company_id\") != merged_rec.get(\"company_id\"):\n"),
 ("the customer is compared with the workbook's row, not the kept record",
  "                    _s(prior.get(\"company_id\")) != _s(merged_rec.get(\"company_id\")):\n",
  "                    _s(prior.get(\"company_id\")) != _s(rec.get(\"company_id\")):\n"),
 ("the PO is compared with the workbook's row, not the kept record",
  "            if fname == \"shipments.json\" and _po_changed(prior, merged_rec):\n",
  "            if fname == \"shipments.json\" and _po_changed(prior, rec):\n"),
 ("an edit no longer on the record is listed as kept",
  "            kept = {f for f in touched if f in prior} - set(dropped)\n",
  "            kept = set(touched) - set(dropped)\n"),
 ("a project gaining a customer prints an empty name",
  "            old = f\"{c.get('old_name') or c['old']} ({c['old']})\" if c[\"old\"] else \"no customer\"\n",
  "            old = f\"{c.get('old_name') or c['old']} ({c['old']})\"\n"),
 ("the report does not say which fields were carried",
  "                    \"carried\": sorted(f for f in OPERATOR_ONLY[fname]\n",
  "                    \"carried\": sorted(f for f in ()\n"),
 ("a leg's vendor-PO change carries its eta anyway",
  "            if fname == \"shipments.json\" and _po_changed(prior, merged_rec):\n", "            if False:\n"),
 ("a PO differing only by case or spacing reads as a different delivery",
  "    return \" \".join(_s(v).split()).casefold()\n", "    return _s(v)\n"),
 ("a leg that loses its PO is taken as the same delivery",
  "    return bool(old) and old != _po(rec.get(\"vendor_po_raw\"))\n",
  "    new = _po(rec.get(\"vendor_po_raw\"))\n    return bool(old) and bool(new) and old != new\n"),
 ("a leg that gains a PO is taken as a different delivery",
  "    return bool(old) and old != _po(rec.get(\"vendor_po_raw\"))\n",
  "    return old != _po(rec.get(\"vendor_po_raw\"))\n"),
 ("a dropped value is reported but still carried",
  "                for f in dropped:\n                    if f in rec:\n                        merged_rec[f] = rec[f]\n"
  "                    else:\n                        merged_rec.pop(f, None)\n", ""),
 ("the report does not give the dropped value",
  "                        \"new_po\": _s(merged_rec.get(\"vendor_po_raw\")), \"dropped\": dropped})\n",
  "                        \"new_po\": _s(merged_rec.get(\"vendor_po_raw\")), \"dropped\": list(dropped)})\n"),
 ("a dropped field is still listed as an edit that was kept",
  "            kept = {f for f in touched if f in prior} - set(dropped)\n",
  "            kept = {f for f in touched if f in prior}\n"),
 ("the printed report omits customer changes",
  "    if report.get(\"company_changed\"):\n", "    if False:\n"),
 ("the printed report omits dropped delivery dates",
