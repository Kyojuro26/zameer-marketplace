...
async function run(crmDir) {
  ...
  r.check('Completed lists it first, most recent first',
    JSON.stringify(a.done1) === JSON.stringify(['acme::4600', 'acme::4700']), JSON.stringify(a.done1));
  r.check('the Completed heading carries the count', /2 completed/.test(a.doneHead1 || ''), a.doneHead1);
