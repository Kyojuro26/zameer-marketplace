...
SERVER = [
 ...
 # re-anchored review round 2: one ASCII whitespace set on both sides
 ("a blank completed_on counts as complete",
  '    return isinstance(v, str) and v.strip(COMPLETED_BLANK) != ""', '    return isinstance(v, str)'),
 ("completion is read as truthiness, so '  ' is complete",
  '    return isinstance(v, str) and v.strip(COMPLETED_BLANK) != ""', '    return bool(v)'),
 ("a completed_on that is not a string counts as complete (round 1)",
  '    return isinstance(v, str) and v.strip(COMPLETED_BLANK) != ""', '    return v is not None and str(v).strip() != ""'),
 ("the server strips Python's whitespace, not the page's (round 2)",
  '    return isinstance(v, str) and v.strip(COMPLETED_BLANK) != ""', '    return isinstance(v, str) and v.strip() != ""'),
...
VIEW = [
 ...
 ("the page trims JavaScript's whitespace, not the server's (round 2)",
  "function isCompleted(p){ const v = p && p.completed_on; return typeof v === 'string' && v.replace(/[ \\t\\n\\r\\f\\v]/g, '') !== ''; }",
  "function isCompleted(p){ const v = p && p.completed_on; return typeof v === 'string' && v.trim() !== ''; }"),
 # re-anchored review round 2: demoProject compares as the server's _key does
 ("demo mode finds a project by number alone (round 1)",
  ...
  "  const hits = DATA.projects.filter(x => k(x.project_no) === k(no));\n"),
 ("demo mode reads a missing customer as 'null', not as no customer (round 2)",
  "  const k = v => v == null ? '' : String(v).trim();\n", "  const k = v => String(v);\n"),
 ("demo mode takes the first of a shared number named without a customer (round 2)",
