...
M = [
 ...
 # re-anchored after the review: the tile also says how many are not verified
 ("QuickBooks' denominator is dropped",
  ...
  "    recvL = `Open receivables \\u00b7 QuickBooks as of ${esc(fmtDate(q.as_of))}${qboUnverified(q)}`\n"),
 ("the as-of date is dropped",
  "    recvL = `Open receivables \\u00b7 QuickBooks as of ${esc(fmtDate(q.as_of))} \\u00b7 ${esc(shapeCaveat(q))}${qboUnverified(q)}`\n",
  "    recvL = `Open receivables \\u00b7 ${esc(shapeCaveat(q))}${qboUnverified(q)}`\n"),
 ("the unverified count is not summed (review)",
  "    out.not_verified += Number(s.customer_not_verified)||0;\n", ""),
 ("the header does not say how many are not verified (review)",
