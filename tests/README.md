...
| `regression/test_vendor_match.py` | the vendor a leg's PO text names, by one rule shared by the backfill script and the importer: the LAST non-payment parenthetical is the token ("(70% Paid)", "(PAID)", "(30%)" are skipped), a match is exact on the normalised name or an alias the operator wrote in `vendor_aliases.json`, never a prefix or a guess; a colliding name is ambiguous, an alias naming nobody is reported; report mode lists every leg without a vendor, never one that has one, tables the unmatched tokens, and writes nothing; `--apply` writes exactly the exact and aliased matches under the store lock with a changelog entry each, never a leg that has a vendor, never a vendor whose company is archived, and a second run writes nothing; the importer files a new leg by the same rule and reviews an unmatched token as `vendor_token_unmatched`; `list_shipments(vendor_id=)`, `get_vendor.open_legs`, and `update_shipment`/`create_shipment` refusing a vendor no record carries or whose company is archived; `vendor_on_time` moves once legs carry vendors. Mutated by `mutate_vendor_match.py` (rule, script, importer, server) |
...
| `regression/test_split_invoice.js` | the Receivables rows on a split-billed project, painted by headless Chromium (`lib/browser.js` → `lib/browser_drive.py`, Python Playwright): no figure per row, the reason in words, and the rendered rows summing to the header. Mutated by `mutate_split_invoice.py` |
