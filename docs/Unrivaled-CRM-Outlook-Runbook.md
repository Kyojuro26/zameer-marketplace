...
_For the setup session on the operator's Windows PC. Gets email flowing both
ways: emails written from the CRM land in the operator's Outlook as drafts, and
recent activity is pulled back onto each customer automatically. Use
**PowerShell**._

## The two directions, in plain terms

- **CRM → Outlook (outbound):** clicking a contact in the CRM creates a **real
  draft** in the operator's Outlook, which they review and send from Outlook.
  The CRM never sends on its own — by design.
...
## Part 1 — One-time Outlook enablement (the operator's PC)

### 1. Register the app in the operator's Microsoft 365 (their IT admin, ~2 min)
...
Set-Content -Path "C:\UnrivaledCRM\store\.secrets\.graph_config.json" -Encoding Ascii -Value '{"client_id": "<client id>", "tenant_id": "<tenant id>"}'
python -m pip install msal requests --quiet
python -u graph_login.py --store "C:\UnrivaledCRM\store"
```
Enter the code at **microsoft.com/devicelogin**, sign in with **the operator's**
...
for the operator's account. This powers the inbound activity view. It's separate from
...
In the operator's Cowork, create one scheduled task — paste this as the request:
...
activity is already current when the operator opens the CRM.
...
Drafts only — nothing is ever sent for the operator. Inbound is **read-only** — Outlook
