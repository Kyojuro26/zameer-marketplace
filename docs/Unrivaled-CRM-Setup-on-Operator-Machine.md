# Unrivaled CRM — Setup on the Operator's Machine (Windows / HP)

_Run top to bottom on the operator's Windows PC during the setup session.
~20–30 min. Everything runs on their machine, and their data stays theirs. Use
**PowerShell** (Start → type "PowerShell" → Enter). The Outlook half is Part 5._

---

## Part 0 — Before you start (have these ready)

- The operator's **latest Sales Tracker workbook** (the .xlsx).
- Nothing to grant for the plugin repo — `Kyojuro26/zameer-marketplace` is
  public and the installer fetches it anonymously. No GitHub account, sign-in,
  or collaborator invite is needed on the operator's side.
- For Outlook (Part 5): their **Microsoft 365 admin** available (~2 min).
...
```
No sign-in required — the repo is public and fetched anonymously.

## Part 3 — Create the data folder + migrate the workbook
...
2. In PowerShell, create the store folder (plain local — NOT OneDrive; sync
   ...
   over a live store — it would overwrite the operator's edits.**
   ```powershell
...
3. Confirm the data is clean (expect 0 bogus, all shipment legs reconciled):
   ...
   ```
4. _(Optional, once the operator has confirmed them)_ apply their
   duplicate-company merges and the D/G rep-name legend now, so records read
   cleanly from day one.

## Part 4 — Point Cowork at the store + run

Env vars never reach the plugin (Claude spawns it with a sanitized
environment) — the store path lives in a **pointer file** in the operator's
home folder.
...
The operator now has a fully working CRM.

## Part 5 — Outlook, both directions (optional but recommended)

Follow **`Unrivaled-CRM-Outlook-Runbook.md`** (Windows steps). In short:
1. Register the app in **the operator's** M365 (their admin) → client + tenant
   IDs.
2. One-time `python -u graph_login.py` device sign-in with **the operator's** mailbox.
...
5. _(If Outlook on)_ click a contact → a real draft appears in the operator's
   Outlook Drafts.
